  ...
  private clickedCountry(country){
    if (country) {
      let res = `
        ...
        `;
      if (country.start) {res = res.concat(`<p class="p-0 m-0"><strong>Start date:</strong> ${country.start}</p>`);}
      if (country.start_reopening) {res = res.concat(`<p class="p-0 m-0"><strong>Start softening date:</strong> ${country.start_reopening}</p>`);}
      if (country.end) {res = res.concat(`<p class="p-0 m-0"><strong>End date:</strong> ${country.end}</p>`);}
      return res.concat(`
      </div>
