...
import { ageRanges, getRegionByAlpha, getSchoolPopulationByAlpha3, getCountryNameByAlpha, monthNames } from '../utils';
...
export class LockdownComponent implements OnInit {
  ...
  public statsHeaders = ["Country", "Start", "End", "Lockdown Status"];
  ...
  async ngOnInit() {
    ...
    this.numberChildrenImpacted = this.getContinentChildrenPopulation(this.schoolClosureRegion, ["0-9", "10-19"]);
  ...
  public schoolClosureChangeLocation(region: string) {
    ...
    this.numberChildrenImpacted = this.getContinentChildrenPopulation(region, ["0-9", "10-19"]);
   ...
   */
  private getContinentChildrenPopulation(region: string, ranges = ageRanges) {
  ...
  public covidVSSchoolChangeRegion(region: string) {
    ...
    const impactedChildren = this.getContinentChildrenPopulation(region, ["0-9", "10-19"]);
    ...
    console.log("active cases per region", this.covidByContinent[region].activeCases);
    console.log("deaths per region", this.covidByContinent[region].deaths);
    console.log('##########################################################')
  }
